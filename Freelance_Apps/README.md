# Freelance Ops — App desktop locale

Outil personnel de prospection freelance : scraping de leads ciblés par
secteur d'activité (artisans, agences immobilières, PME…), génération
d'emails de prospection et suivi des réponses. À usage mono-utilisateur.
100 % local : SQLite + secrets chiffrés via le trousseau Windows. Aucun
service cloud.
...
```powershell
pnpm build:win    # produit dist/Freelance Ops Setup x.y.z.exe
```
